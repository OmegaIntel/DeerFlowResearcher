    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-openbb-bg-widget rounded-lg border border-openbb-border w-[500px]">
        ...
        <div className="flex items-center justify-between p-4 border-b border-openbb-border">
          <h2 className="text-lg font-mono font-semibold text-openbb-text-primary">Create New Dashboard</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-4 space-y-4">
          {/* Dashboard Name */}
          <div>
            <label htmlFor="dashboard-name" className="block text-sm font-mono text-openbb-text-secondary mb-2">
              Dashboard Name
            </label>
            <input
              id="dashboard-name"
              ...
              className="w-full px-3 py-2 bg-openbb-bg-secondary border border-openbb-border rounded text-sm text-openbb-text-primary placeholder-openbb-text-muted focus:outline-none focus:border-openbb-accent font-mono"
              autoFocus
            />
          </div>
          ...
          {/* Default Ticker */}
          <div>
            <label htmlFor="ticker-symbol" className="block text-sm font-mono text-openbb-text-secondary mb-2">
              Ticker Symbol
            </label>
            <input
              id="ticker-symbol"
          ...
          {templates.length > 0 && (
            <div>
              <label className="block text-sm font-mono text-openbb-text-secondary mb-2">
                ...
                {templates.map((template) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <h4 className="text-sm font-mono font-semibold text-openbb-text-primary">
                          {template.name}
                        </h4>
                        {template.description && (
                          <p className="text-xs font-mono text-openbb-text-muted mt-1">
                            {template.description}
                          </p>
                        )}
                      </div>
