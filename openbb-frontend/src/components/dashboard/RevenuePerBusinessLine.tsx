...
import { useRevenueBusinessLineRealTime } from '../../hooks/useRealTimeDataExtended';
...
const RevenuePerBusinessLine: React.FC<RevenuePerBusinessLineProps> = ({ ticker, onTickerChange, onSettings, onRemove }) => {
  ...
  const processBusinessData = () => {
    ...
    // Define colors for product segments
    const segmentColors: { [key: string]: string } = {
      'Mac': '#00BFFF',
      'iPhone': '#FFD700',
      'iPad': '#32CD32',
      'Wearables': '#FF6347',
      'Wearables, Home and Accessories': '#FF6347',
      'Services': '#9370DB',
      'Service': '#9370DB',
      'Software': '#708090',
      'iTunes': '#708090',
    ...
    const datasets = Array.from(segmentNames).map((segment, index) => {
      ...
      if (!color) {
        ...
        const defaultColors = ['#00BFFF', '#FFD700', '#32CD32', '#FF6347', '#9370DB', '#708090', '#FF1493'];
  ...
  if (isLoading) {
    return (
      <div className="bg-openbb-bg-widget rounded-lg border border-openbb-border p-4 h-full flex flex-col animate-pulse">
        ...
        <div className="widget-content flex-1 overflow-hidden min-h-[200px] bg-openbb-bg-hover rounded"></div>
      </div>
    );
  }
...
export default RevenuePerBusinessLine;