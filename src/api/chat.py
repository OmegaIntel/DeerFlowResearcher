    ...
    if token:
        try:
            ...
            try:
                ...
                print(f"DEBUG: Successfully authenticated user: {current_user.email}, ID: {current_user.id}")
            ...
            logger.warning(f"[chat_stream] Failed to create database session for authentication: {e}")
...
async def _astream_workflow_generator(
    ...
    try:
            ...
            try:
                ...
                if isinstance(event_data, dict) and "__interrupt__" in event_data:
                    if not auto_accepted_plan:
                        ...
                        logger.warning(f"[stream:{thread_id}] INTERRUPT triggered: {interrupt_value}")
                        yield _make_event(
                            "interrupt",
                            {
                                "thread_id": thread_id,
                ...
                if isinstance(event_data, tuple):
                    ...
                    print(f"DEBUG: Agent name: '{agent_name}', message content: '{message_chunk.content[:50] if message_chunk.content else 'None'}'")
                    ...
                    if agent_name == "reporter":
                        ...
                        if hasattr(message_chunk, 'content') and message_chunk.content:
                            ...
                            print(f"DEBUG: Captured report chunk, total length now: {len(captured_report_content)}")
                    else:
                        ...
                        if hasattr(message_chunk, 'content') and message_chunk.content and len(message_chunk.content) > 100:
                            print(f"DEBUG: Found substantial content from agent '{agent_name}', length: {len(message_chunk.content)}")
                            # If this looks like a report (substantial content), capture it
                            if not captured_report_content:  # Only if we haven't captured anything yet
                                captured_report_content += message_chunk.content
                                print(f"DEBUG: Captured content from '{agent_name}' as fallback, total length: {len(captured_report_content)}")
                    ...
                    if finish_reason:
                        ...
                        logger.debug(f"[stream:{thread_id}] Finish reason: {finish_reason}")
                    ...
                    elif isinstance(message_chunk, AIMessageChunk) and getattr(message_chunk, 'tool_call_chunks', None):
                    ...
                    logger.debug(f"[stream:{thread_id}] Skipping non-interrupt dict: {event_data}")
        ...
        try:
            ...
            print(f"DEBUG: Captured report content length: {len(captured_report_content)}")
            ...
            if captured_report_content and captured_report_content.strip():
                ...
                print(f"DEBUG: Calling ReportService.save_report_from_state with user_id: {user_id}")
                ...
                if report_id:
                    logger.info(f"[stream:{thread_id}] Report saved successfully with ID: {report_id}")
                    # Include report_id in the stream_end event
                    yield _make_event("stream_end", {
                        "thread_id": thread_id,
                        "finish_reason": "stop",
                        "report_id": report_id
                    })
                ...
                logger.warning(f"[stream:{thread_id}] No report content captured during streaming")
                yield _make_event("stream_end", {
                    "thread_id": thread_id,
                    "finish_reason": "stop"
                })
...
async def _get_final_state_from_graph(thread_id: str, config: dict) -> Optional[dict]:
    ...
    try:
        ...
        if checkpointer:
            ...
            if state_snapshot:
                print(f"DEBUG: State snapshot attributes: {[attr for attr in dir(state_snapshot) if not attr.startswith('_')]}")
                        ...
                        try:
                            ...
                            if hasattr(values, '__iter__'):
                                ...
                                for i, item in enumerate(values_list):
                                    ...
                                    if isinstance(item, dict) and 'final_report' in item:
                                        print(f"DEBUG: Found state dict at index {i}")
                                        print(f"DEBUG: State keys: {list(item.keys())}")
                                        print(f"DEBUG: final_report length: {len(item['final_report']) if item.get('final_report') else 0}")
                                        return item
                                
                                # If no dict with final_report found, return the last dict-like item
                                for item in reversed(values_list):
                                    if isinstance(item, dict):
                                        print(f"DEBUG: Using last dict item as fallback")
                                        print(f"DEBUG: Fallback keys: {list(item.keys())}")
                                        return item
        ...
        logger.exception(f"[get_final_state] Error retrieving state for thread {thread_id}: {e}")
        return None
