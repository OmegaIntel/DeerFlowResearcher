...
class ReportService:
    ...
    def get_user_reports_count(user_id: str, db: Session) -> int:
        ...
        try:
            ...
            logger.exception(
                f"[get_user_reports_count] Error counting reports for user {user_id}: {e}"
            )
            return 0

    @staticmethod
    def get_user_reports_with_names(
        ...
        try:
            ...
            for report in reports:
                report_name = ReportService.extract_report_name(
                    report.report_content, report.plan_title
                )
                result.append(
                    {
                        "id": str(report.id),
            ...
            logger.exception(
                f"[get_user_reports_with_names] Error retrieving reports for user {user_id}: {e}"
            )
            return []
    ...
    def get_report_by_id(report_id: str, user_id: str, db: Session) -> Optional[Report]:
        ...
        try:
            ...
            logger.exception(
                f"[get_report_by_id] Error retrieving report {report_id} for user {user_id}: {e}"
            )
            return None
    ...
    async def save_report_from_state(
        ...
        try:
            ...
            if final_state is None:
                ...
                logger.warning(
                    f"[save_report] final_state is None for thread {thread_id}"
                )
                return None
            ...
            if not isinstance(final_state, dict):
                ...
                logger.error(
                    f"[save_report] final_state is not a dict. Type: {type(final_state)}, Content: {final_state}"
                )
                return None
            ...
            print(
                f"DEBUG: final_report length: {len(final_report) if final_report else 0}"
            )
            print(
                f"DEBUG: final_report preview: {final_report[:100] if final_report else 'None'}"
            )

            if not final_report or final_report.strip() == "":
                print(f"DEBUG: Empty or None final_report, skipping save")
                logger.warning(
                    f"[save_report] Empty report for thread {thread_id}, skipping save"
                )
                return None

            logger.info(
                f"[save_report] Found report content with length: {len(final_report)}"
            )
            ...
            if current_plan:
                ...
                if isinstance(current_plan, Plan):
                    ...
                    try:
                        ...
                        logger.warning(
                            f"[save_report] Could not parse plan JSON for thread {thread_id}"
                        )

            print(f"DEBUG: plan_title: {plan_title}")
            print(
                f"DEBUG: plan_description: {plan_description[:100] if plan_description else 'None'}"
            )
            ...
            print(
                f"DEBUG: observations count: {len(observations) if observations else 0}"
            )
            ...
            try:
                ...
                report = Report(
                    ...
                    auto_accepted_plan=str(
                        request_data.get("auto_accepted_plan", False)
                    ).lower(),
                    enable_background_investigation=str(
                        request_data.get("enable_background_investigation", True)
                    ).lower(),
                ...
                logger.info(
                    f"[save_report] Successfully saved report {report.id} for thread {thread_id}"
                )
                ...
                db.rollback()
                logger.error(
                    f"[save_report] Database error saving report for thread {thread_id}: {e}"
                )
                return None
            except Exception as e:
                ...
                db.rollback()
                logger.error(
                    f"[save_report] Unexpected database error for thread {thread_id}: {e}"
                )
                return None
            finally:
            ...
            logger.exception(
                f"[save_report] Unexpected error saving report for thread {thread_id}: {e}"
            )
            return None

    @staticmethod
    def get_report_by_thread_id(
        ...
        try:
            ...
            logger.exception(
                f"[get_report] Error retrieving report for thread {thread_id}: {e}"
            )
            return None
    ...
    def get_user_reports(
        ...
        try:
            ...
            logger.exception(
                f"[get_user_reports] Error retrieving reports for user {user_id}: {e}"
            )
            return []
